...
2. **Source-maintained path**: prefer the fork-upgrade flow when the plugin is a fork of an active upstream — add an `upstream` remote if only `origin` exists, then per upstream release `git fetch upstream && git merge <tag>`; the conflict file list *is* the customization surface. Self-owned repos (no external upstream) instead get a baseline-adaptation commit driven by the plugin's own typecheck against the new DSH baseline — the same API-alignment probe as Stage 2. Either way: adapt only what the typecheck flags, rebuild only that plugin against the new checkout, verify it loads, and publish as `<name>-<version>-onto-<baseline>.tgz`. Never run routine dependency updates inside plugin repos — bump only what the typecheck demands, because working dependency combinations are the plugin's tested state. A customized copy kept without git history (a downloaded-and-edited tree) needs a one-time migration first: `git init`, commit the pristine upstream version as the base commit, commit the customization diff, add the upstream remote — after that it upgrades by merge like any fork.
